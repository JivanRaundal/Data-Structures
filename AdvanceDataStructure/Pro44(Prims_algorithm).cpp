...
void Graph::Prims()
{
	...
	//icreating visited, distance & from array
	...
	cin>>source;
	source = source - start;
	//NOTE:- here we are consideringsource vertex
	for(i=0; i<n; i++)
	{
		visited[i] = 0;
	...
	while(ne > 0)
	{
		//find the unvisited vertes with minimum distance
		int min = INT_MAX;
		for(i=0; i<n; i++)
		{
		...
		//calculating minimum cost for each edged added in MST

...
class BST
{
	...
	public:
		...
		void show();						//helper
		void Preorder(Node*);				//rec
		void Inorder(Node*);				//rec
		void Postorder(Node*);				//rec
		void min_max();						//helper
		...
		void deleteBST();					//helper to call deleteallnodes
...
void BST::deleteAllNodes(Node * T)
{
	if(T == NULL)
	{
		return;
	}
	deleteAllNodes(T->left);
	deleteAllNodes(T->right);
	delete T;
}

...
class GraphColor
{
	...
	public:
		...
		int getNextVertex()			//will return vertex which is remaining for coloring
		{
			for(int i=0; i<graph.n; i++)
			{
				if(color[i] == 0)
			...
			return -1;		//will return -1 if all vertices are colored
		}
			...
			else
			{
				for(int c=1; c<=M; c++)		//loop through color numbers (M)
				{
					if(canicolor(v, c))		//if true
					{
						...
						if(next != -1)
						{
							colormygraph(next);		//color next vertex using recursion
						}
						else
						{
							colormygraph(graph.n);		// terminating condition
						}

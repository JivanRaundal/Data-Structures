...
void CLL::create()
{
	...
	char str[10];
	cout<<"\nEnter first data of 'STOP' to stop : ";
	cin>>str;
...
void CLL::addbypos(int pos,int x)
{
	...
	else
	{
		...
		q->next = p->next;
		p->next = q;
		if(pos == n+1)	//OR if(p == rear), if pos == n+1 then only rear will change
		{
			rear = q;
		}
	}
...
void CLL::delatbeg()
{
	...
	if(front == rear)	//OR if(front->next = front) i.e. if list contains only 1 node
	{
		delete front;
		front = rear = NULL;
	}

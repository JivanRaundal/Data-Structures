...
void CLL::addbypos(int pos,int x)
{
	...
	else
	{
		...
		if(pos == n+1)	//i.e. if pos==n+1, then only rear will change
		{
			rear = q;
		}
	}
...
void CLL::delatbeg()
{
	...
	if(front == rear)	//OR if(front->next = front) i.e. list contains only 1 node
	{
		delete front;
		front = rear = NULL;
	}
...
void CLL::delbypos(int pos)
{
	...
	else
	{
		...
		p->next = q->next;
		if(q == rear)	//OR if(pos == n) i.e. last node then only rear will change
		{
			rear = p;
		}
		delete q;
